        ...
        <c:choose>
          <c:when test="${order.orderState == 0}">
            <c:if test="${order.retCode == 0000}">
              ...
              <c:if test="${sessionScope.smsParam.sendCount <= 2 }">
              <form action="${pageContext.request.contextPath}/pay/sms.do" name="smsSub" id="smsSub">
                <p>如1分钟内未成功接收短信，请点击<a href="javascript:void(0);" onclick="javascript:openwin(1);" class="js_btn">重新接收</a></p>
              </form>
              </c:if>
            </c:if>
            <c:if test="${order.retCode == 1163}">
              <p>您不必重复下单，请按短信提示完成支付！</p>
            </c:if>
          </c:when>
          <c:otherwise>
            <!-- 错误时出现 -->
...
<script type="text/javascript">
var wait = 5; //停留时间
document.getElementById('buyButton').onclick= function () {return false;};
//document.getElementById('buyButton').style.backgroundColor = "#88A2A8";//变灰
function updateinfo(){
  if(wait == 0){
    document.getElementById('buyButton').innerHTML = "支付完成";
    document.getElementById('buyButton').onclick=function () {openwin(2)};
    //document.getElementById('buyButton').style.backgroundColor = "#0087d1";
  }else{
