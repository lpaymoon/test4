...
/**
 * ******************  类说明  *********************
 * class       :  DirectPayAction
 * @author     :  zhaoyan 
 * description :  后台直连下单V3.0
 * @see        :  
 * @version    :  1.0
 * ***********************************************
 */
public class DirectPayAction extends DirectBaseAction{

	@Override
	protected String getFunCode() {
		return DataDict.FUNCODE_DIRECT_HTXD;
	}
	@Override
	public void processBussiness(RequestMsg requestMsg, ResponseMsg responseMsg){
		...
		if(bankId.equals("XE791000")){
			logInfo("调用小额异步下单接口 bankId:%s",bankId);
			cmd.setBusinessType(DataDict.BUSI_JX_WEB);//江西小额
			orderResp = tradeService.asynOrder(bankId,checkTradeResp, cmd);
		}else{
			cmd.setBusinessType(DataDict.BUSI_BSZL);//标商后台直连下单(0301) 20131017
