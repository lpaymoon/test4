 ...
 * @author     :  zhuoyangyang
 * @version    :  1.0  
 * description :  海南彩票下单增加判断用户包月订购关系判断
 * @see        :                        
 * ************************************************/

public class HncpPayAction extends DirectBaseAction{

	@Override
	protected String getFunCode() {
		return DataDict.FUNCODE_HNCP_XD;
	}
	@Override
	public void processBussiness(RequestMsg requestMsg, ResponseMsg responseMsg){
		...
		//1-2包月订购关系判断（没有包月订购关系则向BOSS请求订购）
		MpspMessage checkUserMonthlyServiceResp = restService.queryUserMonthlyServiceInfo(cmd.getMobileId(), cmd.getMerId());
		if(!checkUserMonthlyServiceResp.isRetCode0000()){		
			MpspMessage monthlyServiceOrderResp = tradeService.sendMonthlyServiceOrder(cmd);
        	if(!monthlyServiceOrderResp.isRetCode0000()){
				logInfo("TradeCheck Result Failed[RetCode]:%s:%s",monthlyServiceOrderResp.getRetCode(),"向BOSS订购包月关系失败");
			}else{
				logInfo("TradeCheck Result Failed[RetCode]:%s:%s",monthlyServiceOrderResp.getRetCode(),"向BOSS订购包月关系成功");
			}
			
		}
		logInfo("TradeCheck Result Success[RetCode]0000:包月订购关系鉴权通过");
		//6-下单	
		MpspMessage orderResp = null;

		cmd.setBusinessType(DataDict.BUSI_HNCP);
		orderResp = tradeService.saveOrder(bankId,checkTradeResp, cmd);
