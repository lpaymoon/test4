...
/**
 * ******************  类说明  *********************
 * class       :  ChannelPayAction
 * @author     :  yangwr 
 * description :  商户直接支付V3.0
 * @see        :  
 * @version    :  1.0
 * ***********************************************
 */
public class ChannelPayAction extends DirectBaseAction{

	@Override
	protected String getFunCode() {
		return DataDict.FUNCODE_QDHFZF;
	}
	@Override
	public void processBussiness(RequestMsg requestMsg, ResponseMsg responseMsg){
		...
		//1-5 支付频率控制
		String checkRateResp = merAuthService.checkChannelPayRate(cmd.getMobileId());
		...
		//5-交易屏蔽模板校验  省略
		//6-支付
		MpspMessage orderResp = tradeService.channelPay(cmd);
		if(!orderResp.isRetCode0000()){
			...
			responseEorr2Mer(requestMsg,responseMsg);
			logInfo("QDZF Result Failed[RetCode]:%s:%s",responseMsg.getRetCode(),"支付失败");
			return;
		}
		logInfo("QDZF Result Success[RetCode]:0000:支付成功");
		//7-组装成功响应
	...
	protected String getRetMessage(ResponseMsg responseMsg){
		...
		String retMsg = ObjectUtil.trim(messageService.getMessage(retCode));
		String retCodeBussi = responseMsg.getRetCodeBussi();
		if(!ObjectUtil.isEmpty(retCodeBussi)){
			if(retCodeBussi.length() > 4){
				retMsg = retCodeBussi.substring(4);
			}
	    }
		return retMsg;
	}
