 ...
 * @see        :                        
 * ************************************************/   
public class WxR4OrderAction extends WxOrderBaseAction {
	private static final Log log = LogFactory.getLog(WxR4OrderAction.class);
	@Override
...
//			            log.info("根据当前下单请求的IMEI,IMSI,ICCID查到在24小时内，最后一笔交易是失败的！不能使用快捷支付");
	...
	private void responseSuccess(MpspMessage mpspMessage,RequestMsg requestMsg, ResponseMsg respMap,long price){
		...
		String versionName=ObjectUtil.trim(requestMsg.getStr(DataDict.WX_REQ_VERSIONNAME));
...
//		String xiaoAmt = messageService.getSystemParam("R4DriectPay.Amt.Xiao");
		...
		boolean flag1=checkSDKversion(versionName,clientVersion);//检查客户端版本是否支持直接支付
